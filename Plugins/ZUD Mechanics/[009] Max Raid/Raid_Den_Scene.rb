...
class MaxRaidScene
  ...
  def raid_SavingPrompt(pkmn)
    ...
    if $DEBUG && Input.press?(Input::CTRL)
      ...
      pbMessage(_INTL("You approached the Berserk Statue before you..."))
      return true
    end
    if !raid_pkmn
      if pbConfirmMessage(_INTL("You must save the game before proceeding. Is this ok?"))
        save_game = true
  ...
  def pbStartScene(pkmn)
    ...
    #---------------------------------------------------------------------------
    textPos = [
      [_INTL("BERSERK STATUE"), 25,  32, 0, BASE, SHADOW],
      [_INTL("Leave"),   403, 154, 0, BASE, SHADOW],
      [_INTL("Examine"),   403, 217, 0, BASE, SHADOW],
  ...
  def pbMaxRaidEntry
    ...
    loop do
      ...
      #-------------------------------------------------------------------------
        ...
        if pbConfirmMessage(_INTL("Proceed with the displayed party?"))
          raid_party = []
          $player.able_party.each_with_index do |pkmn, i|
          ...
          pbFadeOutIn {
            ...
            pbDisposeSpriteHash(@sprites)
            @viewport.dispose
            pbMessage(_INTL("\\me[Max Raid Intro]You examined the Berserk Statue...\\wt[34] ...\\wt[34] ...\\wt[60]!\\wtnp[8]")) #if !$DEBUG
            @pkmn.heal
