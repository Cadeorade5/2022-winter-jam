...
#===============================================================================
def pbTransferWithTransition(map_id, x, y, transition = nil, dir = $game_player.direction, duration: nil)
  ...
  when :DIMENSION
    Transitions.dimension(true, bmp)
  else
...
module Transitions
  ...
  #-----------------------------------------------------------------------------
  #  fade without dimension animation
  #-----------------------------------------------------------------------------
  def self.dimension(reverse = false, bmp = nil)
    return if !$game_player || !$scene.is_a?(Scene_Map)
    bmp = Graphics.snap_to_bitmap if bmp.nil?
    viewport = Viewport.new(0, 0, Graphics.width,Graphics.height)
    viewport.z = 99999
    if reverse
      ...
      dur = Graphics.frame_rate
      dur.times do
        viewport.color.alpha -= 255/dur
        Graphics.update
      end
      viewport.dispose
      return
    ...
    bmp.dispose
    bmp = Bitmap.new(Graphics.width, Graphics.height)
    bmp.fill_rect(0, 0, Graphics.width, Graphics.height, Color.white)
    return bmp
  end
  #-----------------------------------------------------------------------------
  #  calculate delta
